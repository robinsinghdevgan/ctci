...
pair<int, int> nextBigSmallIntWithSameSetBits(int n)
{
    auto nextSmall = [](int n) -> int {
        int countOfSetBits = __builtin_popcount(n);
        if (countOfSetBits == 0)
            return 0;
        --n;
        if (n >= 0)
        {
            while (__builtin_popcount(n) != countOfSetBits)
            {
                if (n < 0)
                    break;
                --n;
    ...
    return make_pair(nextSmall(n), nextBig(n));
}
...
pair<int, int> nextBigSmallIntWithSameSetBits_2(int n)
{
